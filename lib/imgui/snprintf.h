...
#define SNPRINTF_H_
    ...
    #endif
    /** @see snprintf() */
    int custom_vsnprintf(char *string, size_t length, const char *format, va_list args) __attribute__((format(printf, 3, 0)));
    int custom_snprintf(char *string, size_t length, const char *format, ...) __attribute__((format(printf, 3, 4)));
    #ifdef __cplusplus
    }
    #endif
#endif  // SNPRINTF_H_

#ifdef CUSTOM_SNPRINTF_IMPLEMENTATION
/** 
...
int custom_vsnprintf(char *string, size_t length, const char *format, va_list args) {
  struct DATA data;
...
int custom_snprintf(char *string, size_t length, const char *format, ...) {
  int rval;
  va_list args;

  va_start(args, format);
  rval = custom_vsnprintf(string, length, format, args);
  va_end(args);

  return rval;
}

#endif  // SNPRINTF_IMPL_H_
